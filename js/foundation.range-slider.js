!function($, Foundation){
  ...
  Slider.prototype._events = function(){
    var _this = this,
        ...
        offset,
        timer;
    this.$element.on('click.zf.slider touchend.zf.slider', function(e){
    ...
    this.$handle.on('mousedown.zf.slider touchstart.zf.slider', function(e){
      $(window).on('mouseup.zf.slider touchend.zf.slider', function(e){
        e.preventDefault();
        e.stopPropagation();
        offset = e['offset' + (vertical ? 'Y' : 'X')];
        _this._setHandle(offset);
        _this._setFill(offset);
        $(window).off('mouseup.zf.slider touchend.zf.slider mousemove.zf.slider touchmove.zf.slider');
