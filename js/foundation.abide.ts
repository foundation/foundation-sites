...
export class Abide extends Plugin {
   ...
   */
  public requiredCheck($el: JQuery) {
   ...
   */
  public findFormError($el: JQuery) {
    const id = $el[0].id;
   ...
   * @returns {Boolean} Boolean value depends on whether or not attribute is checked or empty
   */
  public findRadioLabels($els: JQuery) {
   ...
   */
  public addErrorClasses($el: JQuery) {
  ...
  public removeRadioErrorClasses(groupName: string) {
   ...
   */
  public validateInput($el: JQuery) {
    let clearRequire = this.requiredCheck($el),
        validated = false,
