...
export class Accordion extends Plugin {
    ...
    _init() {
        ...
        this.$tabs.each(function (idx, el) {
            let $el = $(el), $content = $el.children('[data-tab-content]'), id = $content[0].id || GetYoDigits(6, 'accordion'), linkId = el.id || `${id}-label`;
        ...
        this._checkDeepLink = () => {
            ...
            if (anchor.length) {
                let $link = this.$element.find('[href$="' + anchor + '"]'), $anchor = $(anchor);
    ...
    up($target) {
        ...
        let $aunts = $target.parent().siblings(), _this = this;
...
Accordion.className = 'Accordion'; // ie9 back compat
Accordion.defaults = {
    /**
