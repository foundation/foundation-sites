...
(function($, window, document, undefined) {
  ...
  Foundation.libs.abide = {
    ...
    events: function(scope) {
      ...
      form.off('.abide')
        ...
        .on('blur.fndtn.abide change.fndtn.abide', function(e) {
          ...
          self.validate(this);
          e.stopPropagation();
        }).on('valid', self.on_element_valid)
        .on('invalid', self.on_element_invalid);
    ...
    validate: function(element) {
      ...
      if (this.settings.validate_on_change) {
        var form = $(element).closest('[data-abide]');
        this.validate_form(form[0]);
      }
    ...
    check: function(element) {
      ...
      // is_radio?
      if (type == 'radio') {
        return this.is_valid_radio(element, required)
      } else if (type == 'checkbox') {
        return ($(element).is(':checked') || !required);
      } else {
        if (shouldBeEqual) {
          var isEqual = this.is_equal(element);
    ...
    is_valid_radio: function(el, required) {
