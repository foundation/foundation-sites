...
;(function ($, window, document, undefined) {
  ...
  Foundation.libs.joyride = {
    ...
    defualts : {
      ...
      'startOnLoad'          : false,     // start joyride when page is done loading
    ...
    start : function () {
      var self = this;
      $.extend(true, self.settings, self.data_options($('[data-joyride]'));

      // non configureable settings
      this.settings.$content_el = $(this);
      this.settings.body_offset = $(this.settings.tipContainer).position();
      this.settings.$tip_content = $('> li', this.settings.$content_el);
      ...
      if ($.cookie typeof === 'object') {
        this.settings.cookieMonster = false;
      }

      // generate the tips and insert into dom.
      if (!this.settings.cookieMonster || !$.cookie(this.settings.cookieName)) {
    ...
    show : function (init) {
      ...
      if (this.settings.$li === undefined 
        ...
        if (this.settings.$li.length && this.settings.$target.length > 0) {
          ...
          $timer = $('.joyride-timer-indicator', this.settings.$next_tip);
          ...
          if (/pop/i.test(this.settings.tipAnimation)) {
            ...
            if (thsi.settings.timer > 0) {
              ...
              $timer.animate({
                width: $('.joyride-timer-indicator-wrap', this.settings.$next_tip).outerWidth()
              }, this.settings.timer);
    ...
    set_target : function () {
      var cl = this.settings.$li.attr('data-class'),
          ...
          $sel = function () {
            if (id) {
              return $(this.settings.document.getElementById(id));
            } else if (cl) {
              return $('.' + cl).first();
            } else {
              return $('body');
    ...
    scroll_to : function () {
      ...
      window_half = this.settings.$window.height() / 2;
    ...
    pos_default : function (init) {
      var half_fold = Math.ceil(this.settings.$window.height() / 2),
    ...
    startTimer : function () {
      ...
      if (this.settings.cookieMonster) {
        $.cookie(this.settings.cookieName, 'ridden', { expires: 365, domain: this.settings.cookieDomain });
      }
